    ...
    <section aria-labelledby="comparison-title" className="bg-paper-50 px-5 py-24 text-ink-950 lg:px-8 lg:py-32">
      <div className="mx-auto max-w-6xl">
        <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-end">
          <div><p className="eyebrow text-ink-600">Us versus the default</p><h2 id="comparison-title" className="mt-4 max-w-xl text-4xl font-semibold tracking-[-0.04em] sm:text-5xl">Send the verdict. Keep the account.</h2></div>
        ...
        <div className="mt-14 overflow-hidden rounded-2xl border border-ink-950/15 bg-white shadow-[0_24px_70px_rgba(13,17,15,0.08)]">
          ...
          {rows.map(([label, standard, veilpass]) => (
            <div key={label} className="grid min-h-16 grid-cols-[1.3fr_0.85fr_0.85fr] border-b border-ink-950/10 last:border-0">
              ...
              <div className="grid place-items-center border-l border-ink-950/10 p-4" aria-label={standard ? "Shared" : "Not shared"}>{standard ? <CheckCircleIcon size={22} weight="fill" /> : <XCircleIcon size={22} className="text-ink-400" />}</div>
              <div className="grid place-items-center border-l border-ink-950/10 bg-signal-400/10 p-4" aria-label={veilpass ? "Shared" : "Not shared"}>{veilpass ? <CheckCircleIcon size={22} weight="fill" className="text-signal-700" /> : <XCircleIcon size={22} className="text-signal-700" />}</div>
            </div>
          ))}
        </div>
      </div>
