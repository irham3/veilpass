...
export function DemoBench() {
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[16rem_1fr_22rem]">
      ...
      <SpotlightCard className="rounded-2xl border border-line-dark bg-ink-900 p-5 sm:p-7">
        ...
        <PixelTransition active={Boolean(latest || standard)} first={<div className="grid min-h-72 place-items-center rounded-xl border border-dashed border-line-dark bg-ink-950/60 p-8 text-center"><div><EyeSlashIcon className="mx-auto text-signal-400" size={34} weight="duotone" /><p className="mt-4 font-medium">No host session yet</p><p className="mt-2 max-w-sm text-sm leading-6 text-paper-200">Compare a normal public account payload with a minimized VeilPass response.</p></div></div>} second={<div className="mt-7 min-h-72 rounded-xl border border-line-dark bg-ink-950 p-5"><p className="font-mono text-[0.6875rem] uppercase tracking-[0.12em] text-paper-200">Host received</p><pre data-testid="demo-payload" aria-live="polite" className="mt-4 overflow-x-auto font-mono text-xs leading-6 text-paper-200"><code>{JSON.stringify(standard ? { walletAddress: "GBRPUBLIC7B5E6K2P", network: "TESTNET", balance: "public via Horizon" } : latest ?? {}, null, 2)}</code></pre></div>} className="mt-7" />
        <div className="relative mt-6 grid gap-3 sm:grid-cols-2"><Button type="button" size="lg" variant="outline" onClick={standardLogin}><WalletIcon />Standard wallet login</Button><Button type="button" size="lg" onClick={() => login()}><EyeSlashIcon />Login with VeilPass</Button></div>
      ...
      <aside className="rounded-2xl border border-line-dark bg-ink-900 p-5"><div className="flex items-center justify-between"><h2 className="font-medium">Verification log</h2><span className="font-mono text-[0.6875rem] text-paper-200">last 6</span></div><ol className="mt-5 space-y-3" aria-live="polite">{events.length ? events.map((event, index) => <li key={`${event.label}-${index}`} className="rounded-lg border border-line-dark bg-ink-950 p-3"><div className="flex items-start justify-between gap-3"><p className="text-xs leading-5">{event.label}</p><span className={event.result.ok ? "text-signal-400" : "text-alert-400"}>{event.result.ok ? "PASS" : "REJECT"}</span></div><p className="mt-2 font-mono text-[0.625rem] text-paper-200">{event.result.ok ? event.result.privateAppId : event.result.error}</p></li>) : <li className="text-sm leading-6 text-paper-200">Actions appear here with minimized outcomes. No wallet address is written to this log.</li>}</ol></aside>
    </div>
  );
}
