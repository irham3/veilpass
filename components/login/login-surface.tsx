...
export function LoginSurface({ gateId, state, hostOrigin }: { gateId: string; state: string; hostOrigin: string }) {
  ...
  async function prove() {
    if (!challenge || !window.opener) return;
    setStatus("Creating simulated integration proof");
    const response = await fetch("/api/proof/simulate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(challenge) });
  ...
  return <main className="grid min-h-screen place-items-center bg-ink-950 p-5 text-paper-50"><section className="w-full max-w-md rounded-2xl border border-line-dark bg-ink-900 p-6 sm:p-8"><div className="flex items-center justify-between gap-4"><span className="grid size-11 place-items-center rounded-xl bg-signal-400/10 text-signal-400"><EyeSlashIcon size={24} weight="duotone" /></span><Badge variant="outline" className="border-signal-400/40 text-signal-400">Simulated proof</Badge></div><h1 className="mt-8 text-3xl font-semibold tracking-[-0.035em]">Private login</h1><p className="mt-3 text-sm leading-6 text-paper-200">Prove the local fixture meets <strong className="text-paper-50">{gateId}</strong>. The host receives a scoped ID, not a wallet address.</p><dl className="mt-7 space-y-3 rounded-xl border border-line-dark bg-ink-950 p-4 text-xs"><div><dt className="text-paper-200">Host origin</dt><dd className="mt-1 break-all font-mono">{hostOrigin}</dd></div><div><dt className="text-paper-200">Status</dt><dd className="mt-1" aria-live="polite">{status}</dd></div></dl><Button className="mt-6 w-full" size="lg" disabled={!challenge} onClick={prove}>Continue with fixture credential</Button><p className="mt-4 text-xs leading-5 text-paper-200">This integration adapter is deterministic and forgeable by design. It is not a zero-knowledge proof.</p></section></main>;
}
