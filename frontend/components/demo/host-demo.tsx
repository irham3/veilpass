...
export function HostDemo({ app, label, purpose, accent }: { app: HostApp; label: string; purpose: string; accent: string }) {
  const [status, setStatus] = useState("Checking host session");
  ...
  const hostOrigin = typeof window === "undefined" ? `http://${app}.localhost:3000` : window.location.origin;

  useEffect(() => {
    let active = true;
    void fetch("/api/session", { credentials: "same-origin", cache: "no-store" })
      .then(async (response) => response.ok ? response.json() as Promise<{ authenticated: boolean; privateAppId: string }> : null)
      .then((session) => {
        if (!active) return;
        if (session?.authenticated) {
          setSessionId(session.privateAppId);
          setStatus("Host session verified");
        } else {
          setStatus((current) => current === "Checking host session" ? "No host session" : current);
        }
      })
      .catch(() => {
        if (active) setStatus((current) => current === "Checking host session" ? "Session status unavailable" : current);
      });
    return () => { active = false; };
  }, []);
  ...
  async function signIn() {
    ...
    try {
      ...
      setLogin(result);
      setSessionId(result.privateAppId);
      setStatus("Host session verified");
    } catch (error) {
