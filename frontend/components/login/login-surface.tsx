...
export function LoginSurface({ gateId, state, hostOrigin }: { gateId: string; state: string; hostOrigin: string }) {
  ...
  async function prove() {
    ...
    try {
      ...
      const witnessResponse = await fetch("/api/credentials/witness", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ credential: { gateId: credential.gateId, epoch: credential.epoch, commitment: credential.commitment, credentialSalt: credential.credentialSalt, credentialRoot: credential.credentialRoot, leafNonce: credential.leafNonce, merklePath: credential.merklePath, pathIsRight: credential.pathIsRight, revocationHash: credential.revocationHash, expiresAt: credential.expiresAt, issuerPublicKey: credential.issuerPublicKey, issuerSignature: credential.issuerSignature } }) });
  ...
  return <main className="aperture-field relative grid min-h-screen place-items-center overflow-hidden p-5 text-paper-50"><div aria-hidden="true" className="aperture-ring absolute right-[-12rem] top-[-10rem] size-[30rem] rounded-full opacity-35" /><section className="relative w-full max-w-md rounded-[2.1rem] border border-paper-50/10 bg-paper-50/[0.035] p-1.5 shadow-[0_40px_120px_rgba(0,0,0,0.42)]"><div className="rounded-[1.6rem] bg-ink-900/94 p-6 sm:p-8"><div className="flex items-center justify-between gap-4"><span className="grid size-11 place-items-center rounded-2xl bg-signal-400/10 text-signal-400"><EyeSlashIcon size={24} weight="duotone" /></span><Badge variant="outline" className="rounded-full border-signal-400/40 text-signal-400">Local ZK proof</Badge></div><h1 className="mt-8 text-4xl font-semibold tracking-[-0.05em]">Private login</h1><p className="mt-3 text-sm leading-6 text-paper-200">Prove <strong className="text-paper-50">{gateId}</strong>. The host receives a scoped ID, not a wallet address.</p><dl className="mt-7 space-y-3 rounded-3xl border border-paper-50/10 bg-ink-950 p-4 text-xs"><div><dt className="text-paper-200">Host origin</dt><dd className="mt-1 break-all font-mono">{hostOrigin}</dd></div><div><dt className="text-paper-200">Credential</dt><dd className="mt-1">{credential ? "Available in IndexedDB" : "Not found"}</dd></div><div><dt className="text-paper-200">Status</dt><dd className="mt-1" aria-live="polite">{status}</dd></div></dl>{credential ? <Button className="mt-6 min-h-12 w-full rounded-full" size="lg" disabled={!challenge} onClick={prove}>Continue with local credential</Button> : <Button className="mt-6 min-h-12 w-full rounded-full" size="lg" onClick={enrollThisBrowser}>Enroll this browser</Button>}<p className="mt-4 text-xs leading-5 text-paper-200">{credential ? "The secret stays in this browser. The verifier receives only the circuit proof and public anti-replay inputs." : "Enrollment uses Freighter once. After it completes, this popup resumes the private login automatically."}</p></div></section></main>;
}
