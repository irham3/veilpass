...
Verified on 26 September 2026: 91 Vitest files, 322 passing tests; selected core 100% of 580 statements, 424 branches, 123 functions, and 479 lines; broad V8 inventory 88.35% statements, 84.17% branches, 87.35% functions, and 91.17% lines. A mandatory report check confirms statement hits in all 107 executable modules; aggregate inventory coverage is not 100%. Playwright desktop/mobile scenarios last passed 48/48 on the previous code commit. The dated [test report](evidence/test-report.md) records the current local run and release gate. Re-run the checks for the current commit before release.
