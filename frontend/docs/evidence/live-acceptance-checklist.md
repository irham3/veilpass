...
3. Read the current `premium-holder` root and epoch with `npm run contract:smoke`. The current Testnet root is nonzero; do not replace it at the same epoch solely to match an empty database. Either restore the exact durable Merkle tree that produced that root or, if existing credentials can be invalidated, have the on-chain owner rotate to a fresh epoch with the canonical empty root (`00` repeated 32 bytes). Confirm the database tree and on-chain root match before allowing enrollment. The dashboard's owner-signed contract actions provide the Testnet path.
