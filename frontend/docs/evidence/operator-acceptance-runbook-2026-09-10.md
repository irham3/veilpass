...
> **Dokumen arsip — diperbarui 26 September 2026:** langkah dan status historis di bawah bukan instruksi operasional saat ini. Pemilik wallet telah menyelesaikan enrollment live dan uji login App A (dua kali) serta App B (sekali). Tiga package publik saat ini berada pada npm `0.2.0`; README lokal yang lebih baru disiapkan untuk `0.2.1`. Lihat [panduan demo aktif](demo-end-to-end-guide-2026-09-25.md), [laporan test terbaru](test-report.md), dan [delivery status](delivery-status.md). Jangan gunakan perintah VPT, domain, deployment ID, atau status package lama dari bagian historis.
