...
> **Runbook historis — diperbarui 26 September 2026:** status operasional pada bagian lama di bawah sudah usang. Enrollment dan login live App A 2/2 serta App B 1/1 berhasil; GitHub master dan production docs kini memuat README/docs terbaru; quality/build dan browser CI berwarna hijau. npm tetap `0.2.0`: tag `v0.2.1` lulus `pack:check`, tetapi publish pertama (`@veilpass/shared`) gagal dengan `OIDC permission denied for this action`. npm settings memerlukan owner security-key authentication. Periksa di ketiga package bahwa Trusted Publisher menunjuk `irham3/veilpass`, workflow `release-packages.yml`, environment kosong, serta direct `npm publish` diizinkan; setelah owner menyimpan perubahan, rerun workflow `Release packages` untuk tag `v0.2.1`. Jangan membuat tag baru dan jangan memakai workstation token/OTP. Gunakan [panduan demo](demo-end-to-end-guide-2026-09-25.md), [test report](test-report.md), dan [delivery status](delivery-status.md) untuk status aktif.
...
| Publikasi npm (status 26 Sep 2026) | `0.2.0` public; `0.2.1` tertahan oleh izin OIDC | Workflow `v0.2.1` lulus `pack:check`; publish pertama (`@veilpass/shared`) gagal dengan HTTP 403 `OIDC permission denied for this action`. Lakukan pemeriksaan/otorisasi Trusted Publisher dengan npm owner; rerun workflow tag yang sama setelah perbaikan. |
| Workflow release GitHub | CI quality/browser hijau; publish tertahan | Workflow punya `id-token: write` dan filename yang direkam di runbook (`release-packages.yml`). Error registri menunjukkan izin Trusted Publisher perlu diverifikasi pada masing-masing package; jangan gunakan token workstation sebagai jalan pintas. |
