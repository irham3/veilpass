...
export const docs: Record<string, DocPage> = {
  ...
  contract: { title: "Gate contract", eyebrow: "Stellar testnet", intro: "The Soroban gate registry stores public policy configuration, epochs, issuer keys, and gate state. Routine login remains off-chain.", sections: [
    ...
    { heading: "Local commands", body: "Run contract tests from the repository root. A live deployment requires a funded testnet identity and explicit contract environment values.", code: "npm run contract:test\nstellar contract build --manifest-path contracts/gate-registry/Cargo.toml", language: "bash" },
  ]},
